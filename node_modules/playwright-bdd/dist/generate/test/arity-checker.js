...
exports.ArityChecker = void 0;
const helpers_1 = require("../../gherkin/helpers");
const utils_1 = require("../../utils");
const paths_1 = require("../../utils/paths");
class ArityChecker {
    constructor(featureUri, config) {
        this.featureUri = featureUri;
        this.config = config;
        this.errors = [];
    }
    checkStepDefinitionArity(matchedDefinition, pickleStep, gherkinStep) {
        if (!this.shouldCheckArity(matchedDefinition))
            return;
        const stepFnArgs = matchedDefinition.definition.arity;
        const expectedArgs = this.getExpectedArgs(matchedDefinition, pickleStep);
        if (stepFnArgs >= expectedArgs.min && stepFnArgs <= expectedArgs.max)
            return;
        this.errors.push(this.formatError(matchedDefinition, {
    ...
    shouldCheckArity(matchedDefinition) {
        return matchedDefinition.definition.providedOptions?.arityCheck ?? this.config.arityCheck;
    }
    // eslint-disable-next-line max-params
    formatError(matchedDefinition, step, stepFnArgs, expectedArgs) {
        const stepTextWithKeyword = (0, helpers_1.getStepTextWithKeyword)(step.keyword, step.stepText);
        const stepLocation = `${this.featureUri}:${(0, utils_1.stringifyLocation)(step.location)}`;
        const { definition } = matchedDefinition;
        const definitionLocation = `${(0, paths_1.relativeToCwd)(definition.uri)}:${definition.line}`;
        const expectedStr = expectedArgs.min === expectedArgs.max
...
exports.ArityChecker = ArityChecker;
function isPlaywrightStyle({ definition }) {
    return !definition.isCucumberStyle() && !definition.isDecorator();
}
//# sourceMappingURL=arity-checker.js.map